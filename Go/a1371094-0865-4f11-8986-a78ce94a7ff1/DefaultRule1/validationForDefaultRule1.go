	...
	import (
		...
		"strconv"
		"strings"
		parser "Default_Organization11_Department2_Tenant3_validation/Go/a1371094-0865-4f11-8986-a78ce94a7ff1/DefaultRule1/parser"
	...
	type ValidationListener struct {
		*parser.BaseDefaultRule1Listener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar DefaultRule1;

expression :  label EOF ;

label : LABEL ;

LABEL :SPLITKEY  OWNKEY [A-Za-z]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDefaultRule1Lexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDefaultRule1Parser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "DefaultRule1"
